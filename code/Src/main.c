...
//full-rxtx cplt callback will be triggered every 1024/48000 seconds. 200 will activate a change every ~ 4 seconds
#define FilterChangeTimer 200
...
//Initial IIR low-pass, fs=48kHz, f_cut=1kHz, q=0.707
float iir_coeffs [5] = {
...
//0 = lowpass, 1=hipass, 3=eq
uint8_t filterstate = 0;
uint8_t filtertimer = FilterChangeTimer;
...
void CalcFilters() {
	...
	if (filterstate==0) {
		...
		filterstate = 2;
		//calc EQ - formulas for EQ cut mode; 1 kHz, -15dB, q=0.7
